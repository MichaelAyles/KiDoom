...
This proves that the object pool pattern (pre-allocating traces and reusing
them by updating positions) is the correct approach.
"""
...
def run_benchmark(num_traces=200, num_frames=50):
    ...
    if not board:
        ...
        print("4. Run: exec(open('tests/benchmark_object_pool.py').read())")
        return None
...
if __name__ == '__main__':
    try:
        ...
        if result:
            import sys
            sys.exit(0 if result['speedup'] >= 2.0 else 1)
